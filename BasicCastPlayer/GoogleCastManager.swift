...
class GoogleCastManager: NSObject {
    ...
    private func findPreferredSource(fromSources sources: [BCOVSource], withHTTPS: Bool) -> BCOVSource? {
        ...
        if let hlsSource = hlsSource {
            print("hlsSource")
            print(hlsSource.url.absoluteString)
        ...
        if let mp4Source = mp4Source {
            print("mp4Source")
            print(mp4Source.url.absoluteString)
            return mp4Source
        }
    ...
    private func findDashSource(fromSources sources: [BCOVSource], withHTTPS: Bool) -> BCOVSource? {
        ...
        if let dashSource = dashSource {
            print("dashSource")
            print(dashSource.url.absoluteString)
    ...
    private func createMediaInfo(fromVideo video: BCOVVideo) {
        ...
        source = findDashSource(fromSources: video.sources, withHTTPS: true)
        
        if source == nil {
            source = findDashSource(fromSources: video.sources, withHTTPS: false)
        }
